...
//const q = new ResizingArrayQueue(Infinity);
//const q = new ResizingArrayQueue('asdf');
//const q = new ResizingArrayQueue({});
...
/*
    Resizing Array Queue with enqueue() and dequeue()
*/
i = 0;
console.time(`ResizingArrayQueue enqueue(): ${SIZE} times`);
while(i++ < SIZE) {
    q.enqueue(i);
}
console.timeEnd(`ResizingArrayQueue enqueue(): ${SIZE} times`);


console.time(`ResizingArrayQueue dequeue(): ${SIZE} times`);
while(q.size()) {
    q.dequeue();
}
console.timeEnd(`ResizingArrayQueue dequeue(): ${SIZE} times`);


/*
    library queue with unshift() and shift();
*/
const libraryQ = [];
i = 0;
